...
use crate::rdf::{
    ...
    convert_term_to_named_or_blank_node_ref, 
    get_dataset_node, list_distributions, parse_turtle, QualityMeasurementValue
};
    ...
    match parse_turtle(graph) {
        Ok(store) => {
            match get_dataset_node(&store) {
                Some(dataset_node) => {
                    calculate_score(dataset_node.as_ref(), &store)
                }
...
fn calculate_distribution_score(
    ...
    match event {
        URL =>  {
            if let Some(dl_availability) = rdf::get_quality_measurement_value(dist_node, dcat_mqa::DOWNLOAD_URL_AVAILABILITY, metrics_store) {
                score.accessibility.download_url_availability = if dl_availability == QualityMeasurementValue::Bool(true) {
                    20
                } else {
                    0
                }
            }
            if let Some(dl_status_code) = rdf::get_quality_measurement_value(dist_node, dcat_mqa::DOWNLOAD_URL_STATUS_CODE, metrics_store) {
                if status_code_ok(dl_status_code) {
                    score.accessibility.download_url_status_code = 30;
                }
            }
            if let Some(acc_status_code) = rdf::get_quality_measurement_value(dist_node, dcat_mqa::ACCESS_URL_STATUS_CODE, metrics_store) {
    ...
    list_distributions(dataset_node, metrics_store)
        ...
        .fold(Ok(Score::default()), |max_score, dist_score| {
            match (max_score, dist_score) {
                (Ok(a), Ok(b)) => if a.score() > b.score() { Ok(a) } else {Ok(b)},
                (_, Err(err)) => Err(err),
                (Err(err), _) => Err(err),
            }
        })
